...
const Page = () => {
   ...
   const renderTutorial = () => {
      if (showTutorial) {
         return (
            <>
               <div className={`absolute left-0 top-0 z-[1500] rounded-2xl tutorial-pulsing ${activeElement.id === 0 ? '' : 'transition-transform'}`}
                  style={{
               ...
               <Card className="fixed bottom-8 right-8 z-[1600] max-w-80 rounded-2xl p-3">
                  <div className="flex flex-row justify-between items-center border-b dark:border-neutral-800 mb-3">
                     <Button className="relative -top-1 -left-1" variant="link" onClick={onSkip}>
                        Skip
                     </Button>
         ...
         <div className="mx-auto max-w-5xl pb-10 px-10">
            ...
            <Card className="mt-10">
               <CardContent className="relative min-h-40">
                  <div className="mt-10 max-w-48">
                     <div className="flex justify-between h-16 border-2 bg-white rounded-3xl mb-2 dark:border-neutral-800 dark:bg-transparent" ref={buttons}>
