...
const Page = () => {
   ...
   const renderCharacterCards = () => {
      return characters.map((item) => (
         <button key={item.id} type="button" className="group overflow-hidden transition-all duration-300 hover:z-40 w-28 hover:w-64 focus:overflow-visible hover:overflow-visible focus:z-40 focus:w-64 cursor-pointer">
            <Card key={item.id} className="relative w-full flex items-center border rounded-sm h-96 bg-cover bg-center bg-no-repeat"
               ...
               <Image width={256} height={382} className="absolute z-20 h-full min-w-64 left-1/2 -translate-x-1/2 origin-bottom transition-transform duration-300 group-hover:scale-110 group-focus:scale-110" src={item.image} alt={`${item.name} character`} />
